...
const query: IResolvers = {
  Query: {
    ...
    async racesByYear(_: void, { year }: any, { dataSources }) {
      return await dataSources.races
        .getYear(year)
        .then((data: any) => data.MRData.RaceTable.Races);
    },
    async raceSelect(_: void, { year, round }: any, { dataSources }) {
      return await dataSources.races
    ...
    async circuitSelect(_: void, { year, round }: any, { dataSources }) {
      return await dataSources.circuits
        .getCircuitSelect(year, round)
